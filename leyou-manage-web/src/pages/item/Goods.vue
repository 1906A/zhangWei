    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable==1" @click="putOrOut('0',props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="putOrOut('1',props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteGoods(id) {
        console.log(id);
        this.$message.confirm('此操作将删除数据，是否继续?', '提示', {
          confirmButtonText: '确定删除',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          // alert("删除成功！");                                     //解钩  ==value.data
          this.$http.get("/item/spu/deleteById/" + id).then(({data}) => {
      ...
      putOrOut(x,id) {
        ...
        this.$message.confirm('是否将商品' + name + '?', '提示', {
          confirmButtonText: '确定'+name,
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          // alert("删除成功！");                                     //解钩  ==value.data
          this.$http.get("/item/spu/putOrOut?" ,{
            params : {
              spuId:id,
              saleable : x
            }
